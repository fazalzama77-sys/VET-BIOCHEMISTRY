...
# We can run a node script that loads both the original and temp_test files and compares them
compare_node_code = """
const fs = require('fs');
...
with open("tools/temp_test/verify_no_loss.js", "w", encoding="utf-8") as fp:
    fp.write(compare_node_code)

import subprocess
res = subprocess.run(["node", "tools/temp_test/verify_no_loss.js"], capture_output=True, text=True, encoding="utf-8")
print(res.stdout)
if res.stderr:
