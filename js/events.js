/*
 * Safe renderer for ICAR-IVRI Veterinary Biochemistry department announcements.
...
(() => {
    ...
    const SEEN_KEY = 'vbioc-event-announcements-seen';
    ...
    const normalizeConfig = () => {
        const source = window.IVRI_BIOCHEMISTRY_EVENTS_CONFIG;
        ...
        return {
            ...
            title: safeText(source.sectionTitle, 'Biochemistry Announcements'),
    ...
    const buildCard = (event) => {
        const card = make('article', 'vbioc-event-card');
        if (event.featured === true) card.classList.add('is-featured');

        const meta = make('div', 'vbioc-event-meta');
        meta.append(make('span', 'vbioc-event-category', safeText(event.category, 'Academic Event')));
        if (event.published !== true) meta.append(make('span', 'vbioc-event-draft', 'DRAFT PREVIEW'));

        const title = make('h3', '', safeText(event.title));
        const date = make('p', 'vbioc-event-date');
        date.textContent = `📅 ${formatDate(event.date)}`;

        card.append(meta, title, date);
        const speaker = safeText(event.speaker);
        if (speaker) card.append(make('p', 'vbioc-event-speaker', speaker));
        const description = safeText(event.description);
        if (description) card.append(make('p', 'vbioc-event-description', description));

        const actions = make('div', 'vbioc-event-actions');
        const watch = makeLink(event.youtubeUrl, 'Watch Video', 'vbioc-event-action vbioc-event-youtube', 'fas fa-play');
        const register = makeLink(event.registrationUrl, 'View Details', 'vbioc-event-action vbioc-event-register', 'fas fa-arrow-up-right-from-square');
    ...
    const injectStructuredData = (events) => {
        ...
        const payload = published.map((event) => ({
            ...
            organizer: { '@type': 'Organization', name: 'ICAR-Indian Veterinary Research Institute', url: 'https://veterinarybiochemistry.com/' }
        }));
        const script = document.createElement('script');
        script.type = 'application/ld+json';
        script.dataset.vbiocEvents = 'true';
    ...
    const showAnnouncement = (event) => {
        ...
        if (document.querySelector('.vbioc-event-dialog')) return;

        const overlay = make('div', 'vbioc-event-dialog');
        const dialog = make('section', 'vbioc-event-dialog-card');
        dialog.setAttribute('role', 'dialog');
        dialog.setAttribute('aria-modal', 'true');
        dialog.setAttribute('aria-labelledby', 'vbioc-event-dialog-title');

        const close = make('button', 'vbioc-event-dialog-close');
        close.type = 'button';
        close.setAttribute('aria-label', 'Close announcement');
        close.innerHTML = '&times;';

        dialog.append(close, make('div', 'vbioc-event-dialog-kicker', safeText(event.category, 'Upcoming Biochemistry Event')));
        const title = make('h2', '', safeText(event.title));
        title.id = 'vbioc-event-dialog-title';
        dialog.append(title, make('p', 'vbioc-event-dialog-date', formatDate(event.date)));
        if (safeText(event.description)) dialog.append(make('p', 'vbioc-event-dialog-description', safeText(event.description)));

        const actions = make('div', 'vbioc-event-dialog-actions');
        const watch = makeLink(event.youtubeUrl, 'Open Video', 'vbioc-event-action vbioc-event-youtube', 'fas fa-play');
        const register = makeLink(event.registrationUrl, 'View details', 'vbioc-event-action vbioc-event-register', 'fas fa-arrow-up-right-from-square');
        if (watch) actions.append(watch);
        if (register) actions.append(register);

        const later = make('button', 'vbioc-event-later', 'Not now');
    ...
    const mountEventsSection = () => {
        try {
            ...
            if (!anchor) return;
            if (document.getElementById('vbioc-events')) return;

            const section = make('section', 'vbioc-events-section');
            section.id = 'vbioc-events';
            section.setAttribute('aria-label', config.title);

            const header = make('div', 'vbioc-events-header');
            const headingWrap = make('div');
            const eyebrow = make('div', 'vbioc-events-eyebrow', 'ACADEMIC ANNOUNCEMENTS');
            const title = make('h2', '', config.title);
            headingWrap.append(eyebrow, title);
            if (config.subtitle) headingWrap.append(make('p', 'vbioc-events-sub', config.subtitle));
            header.append(headingWrap);

            const channel = makeLink(config.youtubeUrl, config.youtubeLabel, 'vbioc-events-channel', 'fas fa-video');
            if (channel) header.append(channel);
            section.append(header);

            if (config.events.length) {
                const grid = make('div', 'vbioc-events-grid');
                config.events.forEach((event) => grid.append(buildCard(event)));
                section.append(grid);
            } else {
                const empty = make('div', 'vbioc-events-empty');
                const emptyText = make('div');
                emptyText.append(
                    make('strong', '', config.emptyMessage),
                    make('span', '', 'New seminars, webinars, and biochemistry diagnostic workshops will be posted here.')
                );
            ...
            console.warn('Biochemistry events section skipped:', error);
        }
    };
