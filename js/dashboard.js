/* ============================================================
   ...
   Next-Gen EdTech Learning Dashboard:
     - 0–1000 XP Biochemistry Mastery Index with Circular Gauge & Ranks
...
var dashboardApp = (function () {
  ...
  function getRank(xp) {
    if (xp >= 750) {
      return { title: "Master Veterinary Biochemist", stage: "Phase 4 · Elite Clinical Mastery", icon: "trophy" };
    }
    if (xp >= 500) {
    ...
    return { title: "Biochemistry Apprentice", stage: "Phase 1 · Foundations", icon: "book" };
  }
  ...
  function renderPaperReadiness(syl, readMap, quiz) {
    ...
    var pAnnual = computePaper(["unit-1", "unit-2", "unit-3", "prac-unit-1", "prac-unit-2", "prac-unit-3"], "Annual Examination", "Theory & Practical (All Units)");
    var pTheory = computePaper(["unit-1", "unit-2", "unit-3"], "Theory Assessment", "General, Metabolism & Analytical");
      ...
      return '<div class="paper-gauge-card">' +
        '<div class="paper-card-head">' +
          '<div>' +
            ...
            '<p class="paper-card-subtitle">' + p.totalTopics + ' Topics Syllabus Coverage</p>' +
          '</div>' +
        '</div>' +
    ...
    return '<section>' +
      ...
      '<p class="muted small mt-1">Single board examination following official VCI MSVE 2016 format (Theory 100M + Practical 60M).</p>' +
      '<div class="paper-readiness-grid mt-4">' +
        renderPaperCard(pAnnual, "#/quiz/paper/annual") +
        renderPaperCard(pTheory, "#/quiz/grand") +
      '</div>' +
    '</section>';
  }
  ...
  function renderMatrixFilters() {
    var tabs = [
      { id: "all", label: "All Units (6)" },
      { id: "theory", label: "Theory (3)" },
      { id: "practical", label: "Practical (3)" },
  ...
  function findLowestScoringUnit(allUnits, quiz) {
    ...
    return candidate || { id: "unit-1", name: "General Biochemistry", score: 0, hasScore: false };
  }
